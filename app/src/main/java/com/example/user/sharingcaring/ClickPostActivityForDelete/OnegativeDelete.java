...
public class OnegativeDelete extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_onegative_delete);

        mAuth=FirebaseAuth.getInstance();
        currentUserID=mAuth.getCurrentUser().getUid();

        postKey=getIntent().getExtras().get("PostKey").toString();
        clickPostRef=FirebaseDatabase.getInstance().getReference().child("ONegativeBloodPosts").child(postKey);


        delDescText=findViewById(R.id.about_Oneg_blood_del);
        delLocationText=findViewById(R.id.location_Oneg_blood_del);
        delPhnNumberText=findViewById(R.id.phnNumber_Oneg_blood_del);
        callNowButton=findViewById(R.id.call_now_Oneg_button);
        deletePostButton=findViewById(R.id.delete_Oneg_post_button);
    ...
    private void bloodActivty() {
        Intent intent=new Intent(OnegativeDelete.this,Onegative.class);
        startActivity(intent);
        finish();
    }
